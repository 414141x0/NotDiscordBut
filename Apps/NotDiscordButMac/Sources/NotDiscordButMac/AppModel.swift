...
final class AppModel {
    ...
    var friendsExpanded = true
    var guildsExpanded = true
    var selectedSource: WorkspaceSelection?
    var selectedGuildChannelID: ChannelID?
    var selectedDirectMessageChannelID: ChannelID?
    var railExpanded = true
    var compactRailScrollAnchorID: String?
    var sourceListScrollAnchorID: String?
    var currentChannelListScrollAnchorID: String?
    ...
    @ObservationIgnored
    private var channelListScrollAnchorByGuildID = [String: String?]()

    private var collapsedCategoryIDsByGuildID = [GuildID: Set<ChannelID>]()

    @ObservationIgnored
    private let logger = Logger(subsystem: "disc.notdiscordbut.mac", category: "app")
    ...
    static func makeDefault() -> AppModel {
        ...
        switch profile {
        case .preview:
            return AppModel(client: .preview(), launchProfile: .preview)
        case .live:
            ...
            do {
                ...
                return AppModel(client: client, launchProfile: .live)
            } catch {
    ...
    var people: [WorkspacePerson] {
        filteredPeople(matching: sourceSearchText)
    }

    var guilds: [DiscordGuild] {
        filteredGuilds(matching: sourceSearchText)
    }

    var compactRailMode: CompactRailMode? {
    ...
    var messageSearchPrompt: String {
        if let channel = timeline.selectedChannel?.name {
            return "Search messages in \(channel)"
        }
        if let person = selectedPerson?.displayName {
            return "Search messages with \(person)"
        }
    ...
    var canOpenSelectedGuildWindow: Bool {
        selectedSource?.guildID != nil
    }
    ...
    var secondaryWindowRoute: ConversationWindowRoute? {
        switch selectedSource {
        case let .friend(userID):
            return .directMessage(userID, nil)
        case let .guild(guildID):
            return .guild(guildID, selectedGuildChannelID)
        case nil:
            return nil
        }
    }

    var guildTextChannels: [DiscordChannel] {
        channelListProjection.channels.filter { (channel: DiscordChannel) in
            channel.kind.isTextRenderable
        }
    }

    var guildVoiceChannels: [DiscordChannel] {
        channelListProjection.channels.filter { (channel: DiscordChannel) in
            channel.kind.isVoiceRenderable
        }
    }

    var guildOtherChannels: [DiscordChannel] {
        channelListProjection.channels.filter { (channel: DiscordChannel) in
            !channel.kind.isTextRenderable && !channel.kind.isVoiceRenderable
        }
    }
    ...
    var compactRailRecents: [RecentSelectionEntry] {
        recentSelectionSnapshots.values
            ...
            .compactMap(resolveRecentSelection)
            .filter { entry in
                switch compactRailMode {
                case .friends:
                    return entry.selection.guildID != nil
                case .servers:
                    return entry.selection.userID != nil
                case nil:
                    return false
                }
            }
    ...
    func resolveCaptcha(_ solution: String) async {
        if await authFlow.applyCaptchaSolution(solution, using: client) {
            await hydrateAuthenticatedWorkspace()
        } else {
            phase = .signedOut
        }
    ...
    func startPasswordless() async {
        await authFlow.startPasswordless(using: client)
    }

    func prepareRemoteAuthentication() {
        authFlow.prepareRemoteAuthentication(using: client)
    }
    ...
    func cleanCache() async {
        do {
            ...
            await RemoteAssetCache.shared.clear()
            resetWorkspaceState()
            settings.runtimeNotice = "Local cache cleared."
            settings.cacheStatus = "Cleared"

            if await client.currentSession() != nil {
                statusMessage = "Rehydrating after cache reset…"
                try await client.hydrateHomeState()
                settings.cacheStatus = "Rebuilt"
                await refreshAuthState()
                await refreshSnapshot()
            } else {
                phase = .signedOut
            ...
            settings.runtimeNotice = "Clearing the local cache failed: \(describe(error))"
        }
    }
    ...
    func setSelectedSource(_ selection: WorkspaceSelection?) {
        ...
        clearMessageSearch()
        syncRailForSelection(selection)
        restoreChannelListScrollAnchorIfNeeded()
    ...
    func setRailExpanded(_ expanded: Bool) {
        guard railExpanded != expanded else {
            return
        }
    ...
    func openInspector(for userID: UserID) {
        guard !(compactRailMode != nil && railExpanded) else {
            return
        }
        inspectorUserID = userID
        inspectorProjection = client.state.userInspectorProjection(for: userID, from: latestSnapshot)
    ...
    func updateChannelListScrollAnchor(_ channelIDRaw: String?) {
        currentChannelListScrollAnchorID = channelIDRaw
        guard let guildID = selectedSource?.guildID else {
            return
        }
        channelListScrollAnchorByGuildID[guildID.rawValue] = channelIDRaw
    }
    ...
    private func refreshWorkspace(using snapshot: NormalizedState) {
        ...
        inspectorProjection = inspectorUserID.flatMap {
    ...
    private func syncRailForSelection(_ selection: WorkspaceSelection?) {
        railExpanded = selection == nil
    }

    private func restoreChannelListScrollAnchorIfNeeded() {
        guard let guildID = selectedSource?.guildID else {
            currentChannelListScrollAnchorID = nil
            return
        }
        currentChannelListScrollAnchorID = channelListScrollAnchorByGuildID[guildID.rawValue] ?? selectedGuildChannelID?.rawValue
    }
    ...
    private func filteredPeople(matching query: String) -> [WorkspacePerson] {
    ...
    private func filteredGuilds(matching query: String) -> [DiscordGuild] {
        let trimmedQuery = query.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private func workspacePeople() -> [WorkspacePerson] {
    ...
    private func resetWorkspaceState() {
        ...
        selectedDirectMessageChannelID = nil
        railExpanded = true
        compactRailScrollAnchorID = nil
        sourceListScrollAnchorID = nil
        currentChannelListScrollAnchorID = nil
        inspectorUserID = nil
        inspectorProjection = nil
        inspectorPresented = false
        ...
        channelListScrollAnchorByGuildID.removeAll()
